...
readonly rpm_dir="$(realpath "${1:-${repo_root}/orangepi/dist/zero3/rpms}")"
readonly requested_output="${2:-${repo_root}/orangepi/dist/zero3/Lumina-26.08-OrangePi-Zero3-aarch64.raw.zst}"
...
podman run --rm --security-opt label=disable \
    ...
    "${container_image}" \
    /image/build-rootfs-in-container.sh /rpms "/output/${output_name}" "${image_size}"
