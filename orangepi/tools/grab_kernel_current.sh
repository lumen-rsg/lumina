#!/bin/bash
# Run as ROOT
KVER="6.18.19-current-rockchip64"
STAGE="/tmp/kernel-repack-current"
...
SRC_KERN="/boot/vmlinuz-$KVER"[ -f "/boot/Image" ] && SRC_KERN="/boot/Image"
...
cd $STAGE
tar --owner=0 --group=0 -czvf ~/rpmbuild/SOURCES/kernel-rockchip64-current-6.18.19.tar.gz .

echo "Done! The Current kernel tarball is ready."